...
function parse(argv) {
  const args = { outDir: undefined, cwd: process.cwd(), quiet: false, printJson: false, redact: true, timeoutMs: undefined, handoffFile: undefined, githubSummary: false, command: [] };
  let i = 0;
  for (; i < argv.length; i++) {
    const arg = argv[i];
...
try {
  ...
  const { report, outDir } = await runLogged(cmd, cmdArgs, { cwd: args.cwd, outDir: args.outDir, quiet: args.quiet || args.printJson, redact: args.redact, timeoutMs: args.timeoutMs });
